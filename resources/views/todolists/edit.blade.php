<x-layout.app>
    <main class="flex flex-col w-full min-h-screen max-w-lg mx-auto">
        <section class="bg-[#FFE5CC] pt-12 pb-6 px-6 flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-[#5C3D2E] flex-1">Edit Task</h1>
        </section>

        <section class="bg-white flex-1 px-6 py-6">
            <form action="{{ route('todolists.update', $todolist) }}" method="post" class="space-y-5">
                @csrf
                @method('put')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" placeholder="Enter task description..." rows="6"
                        class="w-full px-4 py-3 border-2 border-[#FFE5CC] rounded-xl focus:outline-none focus:border-[#E59763] focus:ring-2 focus:ring-[#E59763]/20 transition-colors resize-none">{{ old('description', $todolist->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="pending" @selected(old('status', $todolist->status) === 'pending')>Pending</option>
                        <option value="completed" @selected(old('status', $todolist->status) === 'completed')>Completed
                        </option>
                    </select>
                </div>

                <div class="p-4 rounded-xl bg-[#FFF8F0] border border-[#FFE5CC]">
                    <p class="text-sm text-[#8B5A3C] mb-1">Current Status</p>
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <span class="text-[#5C3D2E] font-medium capitalize">{{ $todolist->status }}</span>
                        <span class="text-xs text-[#8B5A3C]">•</span>
                        <span
                            class="text-xs text-[#8B5A3C]">{{ $todolist->created_at->setTimezone('America/Sao_Paulo')->format('d/m/Y') }}</span>
                    </div>
                </div>
